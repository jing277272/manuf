...
def issues(request, project_id):
    if request.method == 'GET':
        ...
        project_total_user = [(request.web.project.creator_id, request.web.project.creator.nickname)]
...
def issues_record(request, project_id, issues_id):
    ...
    if form.is_valid():
        ...
        info = {
            ...
            'creator': instance.creator.nickname,
...
def issues_change(request, project_id, issues_id):
    ...
    def create_reply_record(change_record):
        ...
        new_reply_dict = {
            ...
            'creator': new_object.creator.nickname,
    ...
    if name in ['attention']:
        ...
        if not value:
            ...
            # 获取当前项目所有成员
            user_dict = {str(request.web.project.creator_id): request.web.project.creator.nickname}
            project_user_list = models.ProjectUser.objects.filter(project_id=project_id)

            # 将项目成员全放入user_dict
            for item in project_user_list:
                user_dict[str(item.user_id)] = item.user.nickname
            username_list = []
