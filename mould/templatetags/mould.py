...
def mould_menu_list(request):
    data_list = [
        {'title': '档案', 'url': reverse('dashboard', kwargs={'project_id': request.web.project.id})},
        {'title': '故障', 'url': reverse('issues', kwargs={'project_id': request.web.project.id})},
       # {'title': '统计', 'url': reverse('statistics', kwargs={'project_id': request.web.project.id})},
        {'title': '备件', 'url': reverse('file', kwargs={'project_id': request.web.project.id})},
